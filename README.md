...
### エージェント設定のカスタマイズ（プリセット）

サイドバーの **SETTINGS PRESET** メニューから、エージェントのペルソナ（プリセット）を切り替えることができます。

プリセットの実体は `src/lib/agents/prompts/<プリセット名>/` フォルダです。ここに新しいフォルダを作成し `config.json` とプロンプト・ファイルを配置することで、独自のペルソナを追加できます。

**プリセット設定ファイル (`config.json`) の例:**

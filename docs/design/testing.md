...
graph LR
  ...
  subgraph コンポーネントロジック
    N[clarification-display.ts]
  end
```
...
#### `src/hooks/use-magi-chat.test.ts`

`findLastAssistantMessage` 関数のテスト。メッセージ配列から最後のアシスタントメッセージを検索するロジックの検証。Firebase等の依存はモック化。
...
| ユーザーメッセージのみ | `null` 返却 |
| アシスタント1つ | 該当メッセージ返却 |
| 複数アシスタント | 最後のメッセージ返却（`verdict`, `syncRate` 含む） |
| 末尾がユーザー | 直前のアシスタント返却（`agentResponses`, `contradiction` 含む） |

---

### 14. コンポーネントロジック: Clarification 質問収集

#### `src/components/magi/clarification-display.test.ts`

`collectClarificationQuestions` 純粋関数のテスト。エージェントの応答から clarification 質問を収集するロジックの検証。

| テストケース | 検証内容 |
|:---|:---|
| 空の配列 | 空配列返却 |
| 質問なしレスポンス | 空配列返却 |
| metadata 未定義 | 安全にスキップ |
| 空の質問配列 | 空配列返却 |
| 1エージェント・1質問 | 正しい収集 |
| 1エージェント・複数質問 | 全質問収集 |
| 複数エージェント | 質問ありのエージェントのみ収集 |
| エージェント名対応 | 各質問のエージェント名が正確 |
...
| 日付 | 内容 |
|:---|:---|
| 2026-02-27 | `clarification-display.test.ts` 追加（14テストファイル・115テスト） |
| 2026-02-24 | `use-magi-chat.test.ts` 追加（13テストファイル・110テスト） |
